...
export function ChatListed({ messages, loadingSubmit }: ChatListProps) {
        ...
        <div className="w-full overflow-x-hidden h-full justify-end">
            <div className="w-full flex flex-col overflow-x-hidden overflow-y-hidden min-h-full justify-end">
                {messages.map((message, index) => (
                    <div
                        key={index}
                        className={`flex flex-col gap-2 p-4 ${message.role === 'user' ? 'items-end' : 'items-start'}`}
                    >
                    ...
                    <div className="flex pl-4 pb-4 gap-2 items-center">
                        ...
                        <div className="bg-blue-100 p-3 rounded-md max-w-xs sm:max-w-2xl overflow-x-auto">
                            <div className="flex gap-1">
                                <span className="size-1.5 rounded-full bg-slate-700 animate-bounce"></span>
                                <span className="size-1.5 rounded-full bg-slate-700 animate-bounce delay-150"></span>
                                <span className="size-1.5 rounded-full bg-slate-700 animate-bounce delay-300"></span>
                            </div>
                        </div>
                    </div>
                )}
